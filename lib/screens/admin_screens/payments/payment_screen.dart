...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  List get approvedPayments =>
      payments.where((p) => p["status"] == "paid").toList();

  List get rejectedPayments =>
      payments.where((p) => p["status"] == "rejected").toList();
  ...
  Widget buildPaymentList(List paymentsList) {
    if (paymentsList.isEmpty) {
      return Center(
        child: Text("No payments found", style: AppTextStyles.bodyLarge),
      );
    }
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: TabBarView(
              children: [
                buildPaymentList(pendingPayments),
                buildPaymentList(approvedPayments),
                buildPaymentList(rejectedPayments),
              ],
            ),
          ),
        ],
